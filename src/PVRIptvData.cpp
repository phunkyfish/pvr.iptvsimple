...
PVR_ERROR PVRIptvData::GetMediaAmount(bool deleted, int& amount)
{
  std::lock_guard<std::mutex> lock(m_mutex);
  if (deleted)
    amount = 0;
...
PVR_ERROR PVRIptvData::GetMedia(bool deleted, kodi::addon::PVRMediaResultSet& results)
{
  if (!deleted)
  {
...
PVR_ERROR PVRIptvData::GetMediaTagStreamProperties(const kodi::addon::PVRMediaTag& mediaTag, std::vector<kodi::addon::PVRStreamProperty>& properties)
{
